...
export default function Hero() {
  return (
    <section className="relative isolate flex min-h-[92vh] items-center overflow-hidden bg-brand-black">
      {/* Background layers */}
      <div className="absolute inset-0 -z-10">
        <Image
          src="/images/hero/powder-coating-facility-hero.jpeg"
          alt="Orion Metal Industries fabrication and powder coating facility in Moorabbin"
          fill
          priority
          quality={80}
          ...
          sizes="100vw"
        />
        <div className="absolute inset-0 bg-gradient-to-r from-brand-black via-brand-black/92 to-brand-black/45" />
        ...
        <div className="absolute -left-40 top-1/4 h-[420px] w-[420px] rounded-full bg-brand-red/18 blur-[130px] animate-glow-pulse" />
      </div>

      <HeroVisualWrapper />

      <div className="section-padding container-wide relative w-full">
        <div className="max-w-2xl">
          {/* Location badge */}
          <div className="glass inline-flex items-center gap-3 rounded-full px-4 py-2 animate-fade-up">
          ...
          <h1
            className="mt-8 text-[2.6rem] font-bold leading-[1.04] tracking-tight text-white animate-fade-up sm:text-6xl lg:text-[4.2rem]"
          ...
          <p
            className="mt-7 max-w-xl text-base leading-relaxed text-brand-muted animate-fade-up sm:text-lg"
          ...
          {/* CTAs */}
          <div
            className="mt-10 flex flex-wrap items-center gap-3 animate-fade-up sm:gap-4"
            style={{ animationDelay: "240ms" }}
          >
            <Button href="/contact" variant="primary" size="lg">
              Request a Quote
            </Button>
            <Button href={getWhatsAppUrl()} variant="secondary" size="lg" external icon={false}>
            ...
            <a
              href={getTelUrl()}
              className="group inline-flex items-center gap-2 rounded-full px-4 py-3.5 text-sm font-semibold text-white/70 transition-colors hover:text-white"
            >
          ...
          {/* Capability chips */}
          <div
            className="mt-12 flex flex-wrap gap-2 animate-fade-up"
            style={{ animationDelay: "320ms" }}
          >
            {capabilityChips.map((chip) => (
              <span
                key={chip}
                className="glass rounded-full px-3.5 py-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/70"
              >
                {chip}
              </span>
            ))}
