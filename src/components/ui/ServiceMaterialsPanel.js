 ...
 */
export default function ServiceMaterialsPanel({ section, wide = false }) {
  if (!section?.groups?.length) return null;

  return (
    <div className="glass rounded-2xl border border-white/[0.08] p-5 sm:p-7">
      ...
      <div
        className={`mt-6 grid gap-4 sm:grid-cols-2 ${wide ? "lg:grid-cols-4" : ""}`}
      >
